...
#
#         # --- Convert datetimes and calculate hold time ---
...
# def analyze_trades(uploaded_file):
#     """Analyze trading positions from Excel file (handles timestamps with milliseconds)."""
#     try:
#         df = pd.read_excel(uploaded_file, sheet_name=0, header=None)
#
...
#         })
#
#         # Safely rename columns by index
#         if len(positions_df.columns) > 9:
#             positions_df.columns.values[8] = 'Close Time'
#             positions_df.columns.values[9] = 'Close Price'
#
#         # --- Helper: Flexible datetime parser (supports milliseconds) ---
#         def parse_datetime_flexible(series):
#             parsed = pd.to_datetime(series, format='%Y.%m.%d %H:%M:%S', errors='coerce')
#             if parsed.isna().any():
#                 parsed2 = pd.to_datetime(series, format='%Y.%m.%d %H:%M:%S.%f', errors='coerce')
#                 parsed = parsed.fillna(parsed2)
#             return parsed
#
#         # --- Parse datetime columns ---
#         positions_df['Open Time'] = parse_datetime_flexible(positions_df['Open Time'])
#         positions_df['Close Time'] = parse_datetime_flexible(positions_df['Close Time'])
#
#         # --- Calculate Hold Time ---
#         positions_df['Hold_Time'] = positions_df['Close Time'] - positions_df['Open Time']
#
...
#             curr_symbol = str(positions_df.loc[i, 'Symbol']).strip().upper()
#
#             # Only consider same symbol and valid timestamps
#             if pd.notnull(prev_close) and pd.notnull(curr_open) and prev_symbol == curr_symbol:
#                 time_diff = abs((curr_open - prev_close).total_seconds())
#                 if time_diff <= 20 and (
#                     (prev_type == 'buy' and curr_type == 'sell') or
